...
public class Grid { // defines a 2 dimensional grid as the area in which we will
	...
	public void deleteWord(Word word) { // if word in wordlists, deletes it
		...
		if (word.direction == 2) {
			this.down.remove(word);
			for (int i = 0; i < word.length; i++) { // clear the word
				this.insertChar(word.row+i, word.column, '?');
			}
		}
	}

	public void replaceWord(Word newword, int direction) { // replaces a word of
															// the same row and
															// column and
															// direction
		if (direction == 1) {// across
			for (Word w : across) {
				if (w.row == newword.row && w.column == newword.column) { // if
																			// they're
																			// equal
					across.remove(w);
					across.add(newword);
					for (int i = 0; i < newword.word.length(); i++) {
						...
						this.insertChar(newword.row, newword.column + i,
			...
			for (Word w : down) {
				if (w.row == newword.row && w.column == newword.column) { // if
					...
					for (int i = 0; i < newword.word.length(); i++) {
						...
						insertChar(newword.row + i, newword.column,
								newword.word.charAt(i));
					}
					return; // immediately return to prevent concurrency error
				}
		...
		System.out
				.println("Replacement failed; word doesn't exist at that location");
	}
	...
	public void startGrid() { // adds blank Words to grid
		...
		for (int row = 0; row < length; row++) {
			for (int col = 0; col < length; col++) {
				if (col == 0 || grid[row][col - 1] == '0' && (col != length - 1)) { // if
																					// its
																					// the
																					// start
					// of a word(aka if
					...
					while ((col + wordlength) < length
							&& !(grid[row][col + wordlength] == '0')) { // while
																		// we
																		// don't
																		// hit a
																		// black
				...
				if (row == 0 || grid[row - 1][col] == '0') {
					...
					while ((row + wordlength) < length
							&& !(grid[row + wordlength][col] == '0')) { // while
																		// we
																		// don't
																		// hit a
																		// black
